...
function App() {
  ...
  async function onDownload(items: Item | Array<Item>) {
    ...
    for (const it of items) {
      ...
      const name = it.name.slice(0, it.name.lastIndexOf('.')) + '.cleanup.png'
      downloadBlob(it.cleanup?.base64, name)
      await sleep(100)
    }
  }
  ...
  return (
    <div className="w-full flex flex-col items-center mx-auto px-5">
      <div className="container flex flex-col pt-10 w-full max-w-5xl space-y-12">
        <h1 className="text-2xl font-bold">ClipDrop API - Bulk text removal</h1>
        <APIKeyInput
          value={apiKey}
